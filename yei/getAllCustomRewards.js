...
$project: { // This is where we select the fields that we want returned.
    ...
    fulfiller_email: { $ifNull: [{$arrayElemAt :["$fulfiller.notification_email",0]},"null"]}, //The email address of the FIRST fulfiller. $fulfiller.notification_email returns a list of all fulfillers on this item, but $arrayElemAt selects the 0 indexed item. We didn't want to deal with passing dozens of fulfillers.
    subdomain: { $ifNull: [{$arrayElemAt :["$companies.subdomain",0]},"null"]}, //Subdomain of the company. We don't really need the $ifNull statement, but I was trying to be defensive
    groups: { $ifNull: ["$groups.name","null"]}, //A list of the names of groups who are allowed to access this item
    group_id: { $ifNull: ["$groups._id","null"]}, //A list of the IDs of groups who are allowed to access this item
    categories: { $ifNull: [{$arrayElemAt :["$categories.name",0]},"null"]}, //The FIRST category of the item.
}
}]
).toArray()
